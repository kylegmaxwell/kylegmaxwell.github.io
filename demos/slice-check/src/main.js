 ...
 */
function fetchDcmFile(relativeUrl) {
    let url = relativeUrl;
    if (SAMPLE_SLICE_DATA) {
        const dataUrl = SAMPLE_SLICE_DATA.get(relativeUrl);
        if (dataUrl) {
            url = dataUrl;
        }
    }
...
document.getElementById('meshFile').addEventListener('change', () => {
    loadFiles();
})
